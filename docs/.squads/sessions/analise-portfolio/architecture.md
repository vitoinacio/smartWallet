...
- **Guarda de rota:** Adicionar `OnboardingGuard` em `AppRoutes.tsx`
- **Layout:** OnboardingPage não deve usar LayoutApp. Fullscreen like auth pages.
- **AuthRedirect conflict:** Garantir que OnboardingGuard e AuthRedirect funcionem em cascata sem loop
- **Refresh resilience:** useOnboarding deve ler onboarding.service.ts no mount
- **Skip lógico:** Pular todos os steps ainda deve marcar completo: true com dados default
- **shadcn/ui disponíveis:** Button, Input, Progress, Avatar, Card, Select, Label, Form, Separator
- **Categorias disponíveis:** Reaproveitar lista de categorias do financeiro (TransacaoTypes.ts)
- **Renda com máscara:** Usar input com formatação BRL
